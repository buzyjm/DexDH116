...
Camera-driven teleoperation for the DH116 (LHandPro) 6-DoF dexterous hand.
...
```bash
curl -sSL -o pip.pyz https://bootstrap.pypa.io/pip/pip.pyz
python3 pip.pyz install --target vendor mediapipe opencv-python numpy pyserial
curl -sSL -o models/hand_landmarker.task \
  https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task
```

Nothing is installed system-wide. `rm -rf vendor pip.pyz` undoes all of it.
...
Hold a flat open hand, press SPACE. Make a tight fist, press SPACE. This
records the raw signal values at both extremes into `calibration.json` and
everything in between is interpolated. It warns if the two poses were too
similar for a joint to get useful range.
...
Every pose in `POSE_PLAN` ([poses.py](dexdh116/poses.py)) is one anchor: the
robot's joint angles for it, and your hand's tracker signals for it. Poses added
to the plan after `poses.json` was written appear automatically as unmeasured,
...
|---|---|---|
| `poses` (default) | pose library: your recorded poses blended by similarity ([poses.py](dexdh116/poses.py)) | recorded poses (pinches!) are reproduced exactly |
| `direct` | vendor-style: smooth the 21 landmarks (EMA + Kalman), 2D bend angle per joint, linear map to each actuator ([direct.py](dexdh116/direct.py)) | 1:1, responsive, no anchors needed |
| `hybrid` | `direct` for the four fingers, `poses` for the two thumb joints | responsive fingers + a thumb that still pinches |
| `fk` | kinematic: your fingertip layout is expressed in the robot's palm frame, scaled by palm width, and a damped Gauss-Newton solve finds the six actuator angles whose forward kinematics match it ([fkretarget.py](dexdh116/fkretarget.py)) | any pose has geometric meaning; no anchors to record |

`fk` rests on [kinematics.py](dexdh116/kinematics.py): vendor URDF geometry, the
...
contacts (`touch_grid.json`), and the measured abduction-dependent flexion
...
python3 run.py profile --name li --out handprofile_li.json
python3 run.py teleop --mode fk --hand Right --profile handprofile_li.json
```
...
`handprofile.json` automatically when it exists.
...
`direct_calibration.json`. Tuning: `--lm-alpha` (EMA, vendor 0.3),
...
python3 run.py handview --poses poses.json        # n/N walks the anchors, hjkl orbits
...
| `v` | save the current angle to `.handview_view.json` |
...
DexDH116 session may hold them at a time.
...
```
ERROR: another DexDH116 session is already running: `teleop` (pid 47170, started 14:53:52).
  Stop it with:  python3 run.py stop
```

The running pid is recorded in `.dexdh116.lock`. It has to be the pid of the
...
```
run.py                  entry point (puts vendor/ on sys.path)
dexdh116/
  sdk.py                loads the vendor LHandProLib .so
  hand.py               clamping + rate-limited RS485 control
  tracker.py            MediaPipe landmarks -> 6 raw signals
  retarget.py           calibration, normalization, smoothing, deadband
  poses.py              pose library and blended retargeting
  direct.py             vendor-style per-joint linear retargeting
  session.py            one-instance lock, clean stop
  viz.py                skeleton overlay, hand schematic, joint bars
  teleop.py             check / calibrate / teleop commands
